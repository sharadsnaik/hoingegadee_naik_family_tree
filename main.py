...
app.add_middleware(
     CORSMiddleware,
    allow_origins=['http://localhost:3000','https://naikru.netlify.app', 'https://hoingegadee-naik-family-tree-1.onrender.com'],  #Phone ip],
...
class UpdatePersonModel(BaseModel):
    ...
    spouse_image: Optional[str] = None
    partners_father_name: Optional[str] = None
    partners_mother_name: Optional[str] = None
    spouse_adress: Optional[str] = None
    spouse_siblings: Optional[list] = None

@app.put("/update-person/{uniq_id}")
async def update_person(uniq_id: str, data: UpdatePersonModel):
...
# Saving to the data
@app.post('/submit')
async def submit_form(data: form_data_from_frontend, background_tasks: BackgroundTasks):
...
# Logic for handling if parent add manually childs (especially child is present and parent re-add )
        ...
        background_tasks.add_task(notify_if_needed)


        # Handling the kanndaa tarnslation to collection_name_mongodb_KAN

        async def translate_and_store(document:dict):
        ...
        background_tasks.add_task(translate_and_store, document)
...
async def update_image(uniq_id: str, data: ImageUpdateModel):
    ...
    if data.gender and data.gender.lower() == "male":
        field = "image_url"
    elif data.gender and data.gender.lower() == "female":
        field = "spouse_image"
    else:
        raise HTTPException(status_code=400, detail="Gender must be 'male' or 'female'")

    update_data = {field: data.image_url}
    ...
    # Update OR create Kannada record (safe upsert)
    await collection_name_mongodb_KAN.update_one(
        {"uniq_id": uniq_id},
        {"$set": update_data},
        upsert=True  # This creates it if missing
    )

    return {
        "message": "Image updated & synced to Kannada DB",
        "uniq_id": uniq_id,
        "field": field
    }
...
async def delete_member(uniq_id: str):
    ...
    # Deleting from KAN databses
    await collection_name_mongodb_KAN.delete_one({"uniq_id": uniq_id})
...
async def delete_child(parent_id: str, child_id: str):
    ...
    child_doc = await collection_name_mongodb.find_one({"uniq_id": child_id})
    if child_doc:
        await collection_name_mongodb.delete_one({"uniq_id": child_id})
        await collection_name_mongodb_KAN.delete_one({"uniq_id": child_id})  # Sync delete Kannada!
...
async def delete_all_data():
    ...
    await collection_name_mongodb_KAN.delete_many({})
